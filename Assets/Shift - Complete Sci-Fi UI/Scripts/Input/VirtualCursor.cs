...
namespace Michsky.UI.Shift
{
    public class VirtualCursor : PointerInputModule
    {
        ...
        void Update()
        {
            cursorPos.x += Input.GetAxis(horizontalAxis) * speed * 1000 * UnityEngine.Time.deltaTime;
            cursorPos.x = Mathf.Clamp(cursorPos.x, -+border.rect.width / 2, border.rect.width / 2);

            cursorPos.y += Input.GetAxis(verticalAxis) * speed * 1000 * UnityEngine.Time.deltaTime;
        ...
        public override void Process()
        {
            ...
            if (Input.GetButtonDown("Submit"))
            {
                ...
                pointer.clickTime = UnityEngine.Time.unscaledTime;
